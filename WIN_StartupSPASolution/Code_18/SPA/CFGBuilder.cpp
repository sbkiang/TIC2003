...
CFG* CFGBuilder::buildCFG(Container* procedure) {
	...
	for (int i = startIndex; i < stmts.size() + startIndex; i++) {
		...
		if (!(node->_stmtPtr->_containerHead || node->_stmtPtr->_containerTail)) {
			...
			cout << "node " << node->_stmtPtr->_cfgStmtNum << " sJump : " << node->_sJump->_stmtPtr->_cfgStmtNum << endl;
			continue;
		}
		if (node->_stmtPtr->_container->_type == "procedure") {
			...
			if (!node->_stmtPtr->_containerHead && node->_stmtPtr->_containerTail) { // if stmt is if container tail only
				node->_sJump = _findNextStmt(parentStack, node->_stmtPtr->_cfgStmtNum + 1, stmts);
			}
		...
		//if(node->_sJump){ cout << "node " << node->_stmtPtr->_cfgStmtNum << " sJump : " << node->_sJump->_stmtPtr->_cfgStmtNum << endl; }
		//if (node->_fJump) { cout << "node " << node->_stmtPtr->_cfgStmtNum << " fJump : " << node->_fJump->_stmtPtr->_cfgStmtNum << endl; }
	}
	return cfg;
}
...
void CFGBuilder::_createStmtMap(Container* container, map<int, CFGNode*> &stmtMap) {
	for (int i = 0; i < container->_statements.size(); i++) {
		...
		if (stmt->_cfgStmtNum == (container->_startStmtNum)) {
			stmt->_containerHead = true;
		}
		if (stmt->_cfgStmtNum == (container->_endStmtNum)) {
			stmt->_containerTail = true;
		}
		CFGNode* node = new CFGNode();
		node->_stmtPtr = stmt;
		pair<int, CFGNode*> *mypair = new pair<int, CFGNode*>(stmt->_cfgStmtNum, node);
...
void CFGBuilder::_printStmt(map<int, CFGNode*> stmts) {
	...
	for (map<int, CFGNode*>::iterator it = stmts.begin(); it != stmts.end(); it++) {
		...
		cout << setfill('0') << setw(2) << node->_stmtPtr->_cfgStmtNum << " |";
